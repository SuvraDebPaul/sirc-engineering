...
import { createFeature } from "@/features/content/services/feature-admin";
...
export async function createFeatureAction(data: FeatureInput): Promise<FeatureFormResult | void> {
  ...
  try {
    await createFeature(result.data);
  } catch (error) {
    return { errors: { form: logUnexpectedError("create-feature", error) } };
  }
